...
var BetterFreeEmojis = (() => {
  ...
  var Utils = {
    ...
    Webpack: () => {
      ...
      if (
        ...
        webpackExports =
          typeof Discord.window.webpackJsonp === "function"
  ...
  function Init() {
    ...
    searchHook = Discord.original_searchWithoutFetchingLatest =
      emojisModule.searchWithoutFetchingLatest;
  ...
  function Start() {
    ...
    function replaceEmoji(parseResult, emoji) {
      parseResult.content = parseResult.content.replace(
        `<${emoji.animated ? "a" : ""}:${emoji.originalName || emoji.name}:${
          emoji.id
        }>`,
        emoji.url.split("?")[0] + "?size=48"
      );
    }
  ...
  function Stop() {
    ...
    getEmojiUnavailableReasonHook = Discord.original_getEmojiUnavailableReason;
    BdApi.clearCSS("custom-emoji-css");
  }

  return function () {
    return {
      getName: () => "BetterDiscordFreeEmojis",
      getShortName: () => "BetterFreeEmojis",
      getDescription: () =>
        `Link emojis if you don't have nitro! Type them out or use the emoji picker!

        
        \n\nNOTES:
        \n\nThis plugin is a fork of DiscordFreeEmojis64px.plugin.js by An00nymushun.
        \n\nThis fork fixes the issue where the emoji picker would not show the emoji color.`,
      getVersion: () => "1.8",
      getAuthor: () => "Hoang Huu Giap",
...
module.exports = BetterFreeEmojis;
